...
export const forgotPassword = (email) => async (dispatch) => {
  try {
    ...
    const response = await axios.post(
      "https://my-portfolio-backend-gilt-rho.vercel.app/api/v1/user/password/forgot",
      { email },
  ...
  (token, password, confirmPassword) => async (dispatch) => {
    try {
      ...
      const response = await axios.put(
        ` https://my-portfolio-backend-gilt-rho.vercel.app/api/v1/user/password/reset/${token}`,
